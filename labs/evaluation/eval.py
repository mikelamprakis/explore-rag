...
from labs.evaluation.test import TestQuestion, load_tests
from labs.rag_app.answer import fetch_context, answer_question
...
def _get_judge_prompt(test: TestQuestion, generated_answer: str) -> str:
    return f"""
        Question:
        {test.question}
...
def evaluate_answer(test: TestQuestion) -> tuple[AnswerEval, str, list]:
    ...
    judge_messages = [
        ...
        {"role": "user", "content": _get_judge_prompt(test, generated_answer)},
    ]
...
def evaluate_all_retrieval():
    ...
    for index, test in enumerate(tests):
        result: RetrievalEval = evaluate_retrieval(test)
...
def evaluate_all_answers():
    ...
    total_tests = len(tests)
    for index, test in enumerate(tests):
        result: AnswerEval = evaluate_answer(test)[0]
        progress = (index + 1) / total_tests
        yield test, result, progress


def run_specific_evaluation(test_number: int):
...
if __name__ == "__main__":
    ...
    tests: list[TestQuestion] = load_tests()
    print(f"Loaded {len(tests)} test questions")
    
    example_test: TestQuestion = tests[0]
    print(f"Example test: {example_test.question}")
    print(f"Example test keywords: {example_test.keywords}")
    print(f"Example test reference answer: {example_test.reference_answer}")
    print(f"Example test category: {example_test.category}")
    
    from collections import Counter
    count = Counter[str]([test.category for test in tests])
    print(f"Category count: {count}")
    
    
    retrieval_eval: RetrievalEval = evaluate_retrieval(example_test)
    print(f"Retrieval evaluation: {retrieval_eval}")
    
    answer_eval, generated_answer, retrieved_docs = evaluate_answer(example_test)
    print(f"Answer evaluation: {answer_eval}")
    print(f"Generated answer: {generated_answer}")
    [print(doc.metadata) for doc in retrieved_docs]
    
    
    print("Evaluating all retrieval tests...")
    evaluate_all_retrieval()
    [print(f"RetrievalEval Result {result} - Progress: {progress:.2%}") for test, result, progress in evaluate_all_retrieval(tests)]
    
    print("Evaluating all answers to tests...")
    evaluate_all_answers()
    [print(f"AnswerEval Result {result} - Progress: {progress:.2%}") for test, result, progress in evaluate_all_answers(tests)]
    
    print("Running specific evaluation for test ...")
    run_specific_evaluation(1)
